...
test.describe.fixme('command-layer cancel', () => {
    test.beforeEach(async ({ page: _ }) => {
        await setupMockMcp({ mode: 'happy' });
    });

    test.fixme('TODO T-06: cancel pending → confirm/reject', async ({ page: _ }) => {
        // outline: see docs/interactive-flow/closure-plan.md Appendix B T-06
    });
});
