...
test.describe.fixme('command-layer cancel-ttl', () => {
    test.beforeEach(async ({ page: _ }) => {
        await setupMockMcp({ mode: 'happy' });
    });

    test.fixme('TODO T-07: pending_cancel TTL expiry', async ({ page: _ }) => {
        // outline: see docs/interactive-flow/closure-plan.md Appendix B T-07
    });
});
