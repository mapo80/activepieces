...
test.describe.fixme('command-layer saga-recovery', () => {
    test.beforeEach(async ({ page: _ }) => {
        await setupMockMcp({ mode: 'happy' });
    });

    test.fixme('TODO T-12: API restart → prepared turn recovery', async ({ page: _ }) => {
        // outline: see docs/interactive-flow/closure-plan.md Appendix B T-12
    });
});
