...
test.describe.fixme('command-layer cas-conflict', () => {
    test.beforeEach(async ({ page: _ }) => {
        await setupMockMcp({ mode: 'happy' });
    });

    test.fixme('TODO T-11: two parallel sessions hit CAS conflict', async ({ page: _ }) => {
        // outline: see docs/interactive-flow/closure-plan.md Appendix B T-11
    });
});
